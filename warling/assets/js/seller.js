...
document.addEventListener('DOMContentLoaded', function() {
    ...
    authForm.addEventListener('submit', async function(e) {
        ...
        try {
            let result;
            if (isLoginMode) {
                result = await supabase.rpc('check_pin', { seller_name: name, plain_pin: pin });
            } else {
                result = await supabase.rpc('signup_seller', { shop_name: name, plain_pin: pin });
            }
    ...
    addProductForm.addEventListener('submit', async function(e) {
        ...
        const { count } = await supabase.from('products').select('*', { count: 'exact', head: true }).eq('seller_id', currentSellerId);
        if (count >= 10) {
